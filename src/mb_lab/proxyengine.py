...
from . import algorithms, file_ops, object_ops
...
class ProxyEngine:
    ...
    def fit_proxy_object(self, proxy_offset=0.0, proxy_threshold=0.5, create_proxy_mask=False, transfer_w=True,
                         reverse=False, all_faces=False, smoothing=True):
        scn = bpy.context.scene
        status, proxy, body = self.get_proxy_fitting_ingredients()
        if status == "OK":
            ...
            if not reverse:
                if smoothing:
                    ...
                    correct_smooth_mod = object_ops.new_modifier(proxy, self.corrective_modifier_name,
                                                                 'CORRECTIVE_SMOOTH', parameters)

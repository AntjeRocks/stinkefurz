...
class MaterialEngine:
    ...
    def save_texture(self, filepath, shader_target):
        ...
        img = file_ops.get_image(img_name)

        # Check if the image is None or lacks image data
        if img is None or not img.has_data:
            # Log that the primary image was not valid
            logger.warning("Image '%s' does not contain image data. Trying fallback.", img_name)

            # Try to find a fallback image
            img_fallback_name = img_name + ".001"
            img = file_ops.get_image(img_fallback_name)

            # If fallback image is found and valid, update img_name
            if img and img.has_data:
                logger.info("Using fallback image '%s'", img_fallback_name)
                img_name = img_fallback_name
            else:
                raise RuntimeError(f"Error: Neither '{img_name}' nor '{img_fallback_name}' contain valid image data.")

        # Proceed to save the image
        file_ops.save_image(img_name, filepath)

        # Load the just saved image to replace the current one
        file_ops.load_image(filepath)

        # Update the image file name in the shader target
        self.image_file_names[shader_target] = os.path.basename(filepath)

...
def check_mesh(obj):
    ...
    if not result:
        logger.debug("check_obj %s vertex/face count mismatch: %d %d %d %d", obj.name, len(obj.data.vertices),
                     tpl_config.get("vertices"), len(obj.data.polygons), tpl_config.get("faces"))
    return result

...
forbidden_directories = ["__pycache__", "assets", "mb-lab_updater", "animations", "anthropometry",
...
def save_config():
    global config_content
    if config_content["data_directory"] == "" or config_content["data_directory"] == "assets":
        return
    addon_directory = os.path.dirname(os.path.realpath(__file__))
    file_name = os.path.join(addon_directory, config_content["data_directory"],
                             config_content["data_directory"] + "_config.json")
    ...
    if key == "data_directory":
        ...
        if not key in config_content:
            config_content[key] = {
                ...
                "texture_roughness": "", "texture_melanin": "",
                "texture_sclera_color": "", "texture_thickness": "",
...
def get_presets_folder_list():
    ...
    return [('NONE', 'Unknown', 'Unknown folder for the moment')] + algorithms.create_enum_property_items(dir_list,
                                                                                                          tip_length=20)

...
def get_configuration():
    ...
    if configuration_done is not None:
    ...
    for tup in get_root_directories():
        ...
        if not os.path.isdir(data_path):
            logger.error("get_configuration: %s is not a directory!", data_path)
            continue
        tmp = {}
...
def save_image(name, filepath, fileformat='PNG'):
    ...
    if img:
        print("image will be saved: " + img.source)

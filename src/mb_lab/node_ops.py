...
def get_image_shader(nodes, node_name):
    ...
    return np.array(
        [node_name, texture, col, ext, bc, bf, bt, bright, contrast, saturation, ucr, cm1, alpha1, col1, cpos1, cm2,
         alpha2, col2, cpos2, h_int, intp, fs, fc, fd, fo, ml, mp, mv, uaf, ucy], dtype=object)


def set_image_shader(node_name, texture, col, ext, bc, bf, bt, bright, contrast, saturation, ucr, cm1, alpha1, col1,
                     cpos1, cm2, alpha2, col2, cpos2, h_int, intp, fs, fc, fd, fo, ml, mp, mv, uaf, ucy):
    material = bpy.context.object.active_material
    nodes = material.node_tree.nodes
    node = nodes.get(node_name)
    node.image = texture
